...
path = "C:/Users/Patricia Sittikul/SharePoint/Info LincLV/Shared/1 Clients/Active/CT/Takacova, Maria and Martin, Takac"
os.chdir(path)
print(os.getcwd())

wb = openpyxl.load_workbook('_Maria Takacova Client Log.xlsx')
...
for rowNum in range(2, sheet.max_row + 1):
    ...
    if error1 in str(category):
        ...
        category = 'Resources - Individual Connections'
        sheet.cell(row=rowNum, column=6).value = category
        Matrix[5][rowNum] = category
    elif error5 in str(category):
        category = 'Resources - Events'
        sheet.cell(row=rowNum, column=6).value = category
    else: Matrix[5][rowNum] = category
    wb.save('_Maria Takacova Client Log.xlsx')
...
"""
print(os.getcwd())
resultWB = openpyxl.load_workbook('takacovaimport.xlsx')
...
for num in range(2, int(len(Matrix))):
...
resultWB.save('takacovaimport.xlsx') """
